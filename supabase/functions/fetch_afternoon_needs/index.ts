// AideIci — fetch_afternoon_needs
// Cron: tous les jours à 14h
// Cible: sources type 'asso' ou 'reseau_local'
...
Deno.serve(async (_req) => {
  ...
  for (const source of (sources as Source[]) || []) {
    try {
      ...
      if (!res.ok) {
        console.warn(`[fetch_afternoon] ${source.name}: HTTP ${res.status}`);
        continue;
      }

      const rawNeeds: RawNeed[] = await res.json();
      if (!Array.isArray(rawNeeds)) {
        console.warn(`[fetch_afternoon] ${source.name}: réponse non-tableau`);
        continue;
      }
      ...
      if (insertErr) {
        console.error(`[fetch_afternoon] ${source.name}: insert error — ${insertErr.message}`);
      } else {
        inserted += rows.length;
      }
    } catch (err) {
      console.error(`[fetch_afternoon] ${source.name}: ${err.message}`);
    }
  }
